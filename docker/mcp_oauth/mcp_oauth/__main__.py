...
def main():
    ...
    mcp_servers_config_path = os.getenv('MCP_SERVERS_CONFIG_PATH', args.mcp_servers_config_path)
    refresh_groups_interval = int(os.getenv('MCP_OAUTH_REFRESH_GROUPS_INTERVAL', args.refresh_groups_interval))
    refresh_users_interval = int(os.getenv('MCP_OAUTH_REFRESH_USERS_INTERVAL', args.refresh_users_interval))
    ...
    try:
        ...
        logger.error("Make sure GOOGLE_OAUTH environment variable is set with valid JSON")
        sys.exit(1)
    ...
    try:
        ...
        logger.warning("Group pre-loading will be skipped - this may impact performance")
