...
from ..utils.helpers import check_req_env_vars, validate_google_oauth_token, validate_google_oauth_token_with_client_check
...
class ValidationResult:
    ...
    is_valid: bool = False
    error_type: Optional[str] = None  # "invalid_token", "invalid_client", "workspace_denied", "group_access_denied", "group_validation_error"
...
class GoogleWIF:
    ...
    def __init__(self):
        ...
        required_env_vars = ['SA_EMAIL', 'GCP_SECRET_ARN']
        check_req_env_vars(required_env_vars)

        self.sa_email: str = os.getenv('SA_EMAIL', '')
        self.google_api_auth_scopes: List[str] = ['https://www.googleapis.com/auth/cloud-platform']
        ...
        try:
            ...
            if not gcp_secret_json.strip():
                raise ValueError("GCP_SECRET_ARN environment variable is empty or not set")
            ...
            auth_logger.info(f"Successfully loaded Google WIF configuration for audience: {self.sa_info.get('audience', 'unknown')}")
            
        except json.JSONDecodeError as e:
            auth_logger.error(f"Failed to parse GCP_SECRET_ARN as JSON: {e}")
            raise ValueError(f"Invalid JSON in GCP_SECRET_ARN environment variable: {e}")
            ...
            raise ValueError(f"Failed to load Google WIF configuration: {e}")
    ...
    def _force_credential_refresh(self) -> None:
        ...
        try:
            ...
            auth_logger.warning('Failed to force credential refresh (non-critical): %s', e)
        finally:
            self._credential_refresh_lock = False
    ...
    def __generate_oauth2_client_credentials(self) -> None:
        ...
        for attempt in range(max_retries):
            try:
                auth_logger.info('Starting impersonation (attempt %d/%d)', attempt + 1, max_retries)
                ...
                auth_logger.debug('AWS credentials obtained - access_key: %s..., has_token: %s', 
                ...
                if hasattr(aws_credentials, '_expiry_time') and getattr(aws_credentials, '_expiry_time', None):
                    try:
                        expiry_time = getattr(aws_credentials, '_expiry_time')
                        time_until_expiry = expiry_time - datetime.now(timezone.utc)  # type: ignore[operator]
                        auth_logger.info('AWS credentials expire in: %s', time_until_expiry)
                        
                        # Proactively refresh if credentials expire soon (within 5 minutes)
                        if time_until_expiry.total_seconds() < 300:  # 5 minutes
                            auth_logger.warning('AWS credentials expire soon (%s), forcing complete credential refresh', time_until_expiry)
                            try:
                                ...
                                if aws_credentials:
                                    frozen_creds = aws_credentials.get_frozen_credentials()
                                    auth_logger.info('Successfully performed complete credential refresh for expiring credentials')
                                else:
                                    auth_logger.warning('Complete credential refresh succeeded but no credentials available')
                            except Exception as proactive_refresh_error:
                                auth_logger.warning('Proactive complete credential refresh failed: %s', proactive_refresh_error)
                        
                    except Exception as exp_error:
                        auth_logger.debug('Could not determine credential expiry: %s', exp_error)
                
                os.environ[environment_vars.AWS_ACCESS_KEY_ID] = frozen_creds.access_key
                os.environ[environment_vars.AWS_SECRET_ACCESS_KEY] = frozen_creds.secret_key
                os.environ[environment_vars.AWS_SESSION_TOKEN] = frozen_creds.token

                # Sign JWT using iamcredentials endpoint
                authed_session = AuthorizedSession(AWSCredentials.from_info(self.sa_info).with_scopes(self.google_api_auth_scopes))
                ...
                access_token = self.__generate_domain_wide_delegation_access_token(signed_jwt)
                ...
                if self.__is_credential_expired_error(e) and attempt < max_retries - 1:
                    auth_logger.warning('AWS credentials expired on attempt %d/%d, forcing credential refresh: %s', 
                    ...
                    auth_logger.error('Failed to generate OAuth2 credentials after %d attempts: %s', attempt + 1, e)
                    raise
    ...
    def __build_domain_wide_delegation_jwt(self, service_account: str, subject: str, scopes: List[str], lifetime: int) -> str:
    ...
    def __get_users(self, org_unit_path: str = '/People') -> None:
        ...
        if last_refresh and (now - last_refresh).total_seconds() < self.users_refresh_interval:
            logger.info("✅ USERS: Skipping user refresh for %s - last refresh was %d seconds ago (interval: %d seconds)", 
                        org_unit_path, (now - last_refresh).total_seconds(), self.users_refresh_interval)
            return
        ...
        try:
            ...
            logger.info("Fetching Google Workspace users for org unit: %s", org_unit_path)
            ...
            logger.info("Successfully fetched %d users for org unit: %s", user_count, org_unit_path)

        except Exception as e:
            logger.warning("Error fetching users for %s. The error was %s", org_unit_path, e)

        # Always update the google_users regardless of success/failure
        logger.debug(f'Completed user refresh for organizational unit path: {org_unit_path}')
    ...
    def __initialize_groups(self) -> None:
        ...
        logger.info("Initialized empty groups lookup dictionary (will be populated by server startup process)")
    ...
    def refresh_users(self, org_unit_path: str = '/People') -> None:
        ...
        try:
            ...
            logger.info(f"✅ Successfully refreshed users for org unit: {org_unit_path}")
        except Exception as e:
    ...
    async def __get_user(self, user_identifier: str, org_unit_path: str = '/People') -> UserInfo:
        ...
        if self.google_users is not None:
            ...
            if user_data:
                ...
                auth_logger.info("✅ USER LOOKUP HIT: Found user %s in lookup", user_identifier)
                user = user_data
                is_valid = True
            else:
                ...
                if not last_refresh or (now - last_refresh).total_seconds() >= self.users_refresh_interval:
                    auth_logger.warning("User not found in Google Workspace: %s. Attempting to refresh user data", user_identifier)
                    self.__get_users(org_unit_path)
                    # Check again after refresh
                    if self.google_users is not None:
                        user_data = self.google_users.get(user_identifier)
                        if user_data:
                            auth_logger.debug("Found user %s after refreshing google users", user_identifier)
                            user = user_data
                            is_valid = True
                        else:
                            logger.warning("User not found in Google Workspace: %s (even after refresh)", user_identifier)
                else:
                    time_until_refresh = self.users_refresh_interval - (now - last_refresh).total_seconds()
                    logger.warning("User %s not found in lookup. Next data refresh allowed in %.0f seconds", 
                                 user_identifier, time_until_refresh)
        else:
        ...
        auth_logger.info("Validated Google user: %s (%s). Is user valid? %s", user_identifier, name, is_valid)
    ...
    async def __get_group_members(self, groups: List[str], force_refresh: bool = False) -> Dict[str, List[str]]:
        """
        ...
        for group in groups:
            ...
            if (self.google_team_groups is not None and 
                group in self.google_team_groups):
                
                if last_refresh and (now - last_refresh).total_seconds() < self.groups_refresh_interval:
                    # Fresh data for this group
                    current_groups[group] = self.google_team_groups[group]
                    logger.info("✅ FRESH DATA: Using current group data for group: %s (refreshed %ds ago)", 
                               group, int((now - last_refresh).total_seconds()))
                elif not force_refresh:
                    # We have data and we're not forcing refresh - use current data
                    stale_groups[group] = self.google_team_groups[group]
                    if last_refresh:
                        logger.warning("⚠️ STALE DATA: Using older group data for group: %s (refreshed %ds ago, interval %ds) - will not refresh on individual request", 
                                     group, int((now - last_refresh).total_seconds()), self.groups_refresh_interval)
                    else:
                        logger.warning("⚠️ STALE DATA: Using group data for group: %s (no timestamp) - will not refresh on individual request", group)
                else:
                    # force_refresh=True and we have stale data - need to refresh
                    groups_to_refresh.append(group)
                    if last_refresh:
                        logger.info("🔄 FORCE REFRESH: Group %s data expired (%ds old, interval %ds), force refreshing", 
                                   group, int((now - last_refresh).total_seconds()), self.groups_refresh_interval)
                    else:
                        logger.info("🔄 FORCE REFRESH: Group %s has no timestamp, force refreshing", group)
            else:
                # Group not in lookup at all
                groups_to_refresh.append(group)
                logger.info("❌ LOOKUP MISS: Group %s not in lookup, will refresh", group)
        ...
        if not groups_to_refresh:
            if current_groups and not stale_groups:
                logger.info("✅ ALL FRESH DATA: All requested groups found in fresh data: %s", groups)
            elif stale_groups and not current_groups:
                logger.info("⚠️ ALL STALE DATA: All requested groups found in stale data: %s", groups)
            else:
                logger.info("📋 MIXED DATA: Found %d fresh, %d stale groups from data: %s", 
                           len(current_groups), len(stale_groups), groups)
        ...
        assert self.google_team_groups is not None, "Groups dictionary should be initialized at startup"
        ...
        try:
            ...
            logger.info("Refreshing Google Workspace group members for groups: %s", groups_to_refresh)
            ...
            for group in groups_to_refresh:
                try:
                    ...
                    if not workspace_domain and '@' in self.impersonation_email:
                        workspace_domain = self.impersonation_email.split('@')[1]
                    
                    group_key = f"{group}@{workspace_domain}" if workspace_domain else group
                    logger.debug("Fetching members for group: %s", group_key)
                    
                    # Use type: ignore for Google API dynamic attributes
                    group_members = self.admin_service.members().list(  # type: ignore[attr-defined]
                        groupKey=group_key
                    ).execute().get('members', [])
                    
                    if group_members:
                        member_emails = [member['email'] for member in group_members if 'email' in member]
                        self.google_team_groups[group] = member_emails
                        result[group] = member_emails
                        logger.info("Found %d members in group %s", len(member_emails), group)
                    else:
                    ...
                    logger.warning("Failed to fetch members for group %s: %s", group, group_error)
            ...
            if groups_to_refresh:
                logger.info("Successfully completed group member refresh for %d groups", len(groups_to_refresh))
    ...
    async def get_group_members(self, group: str, user: str) -> Dict[str, bool]:
        ...
        if (self.google_team_groups and 
            group in self.google_team_groups and 
            user in self.google_team_groups[group]):
            is_member = True
    ...
    async def check_user_groups(self, user: str, groups: List[str]) -> Dict[str, bool]:
        ...
        result = {}
        for group in groups:
            is_member = (self.google_team_groups and 
                        group in self.google_team_groups and 
                        user in self.google_team_groups[group])
    ...
    async def validate_oauth_token_with_groups(
        ...
        try:
            ...
            groups_passed = [group for group, is_member in group_membership.items() if is_member]
            
            if require_all_groups:
                # User must belong to ALL required groups
                groups_failed = [group for group in required_groups if not group_membership.get(group, False)]
                if groups_failed:
                    auth_logger.warning("User %s missing required groups: %s", user_email, groups_failed)
                ...
                if not groups_passed:
                    auth_logger.warning("User %s does not belong to any required groups: %s", user_email, required_groups)
                    return ValidationResult(
                        is_valid=False,
                        error_type="group_access_denied", 
            ...
            auth_logger.info("User %s passed group validation. Groups: %s", user_email, groups_passed)
            
            return ValidationResult(
                user_info=enhanced_user,
                is_valid=True
            )
    ...
    async def validate_oauth_token(self, access_token: str, org_unit_path: str = '/People', expected_client_id: Optional[str] = None) -> ValidationResult:
        ...
        if expected_client_id:
            auth_logger.debug("Validating OAuth token with client ID check: %s", expected_client_id)
            validation_result = await validate_google_oauth_token_with_client_check(access_token, expected_client_id)
            if not validation_result:
                logger.warning("OAuth token validation failed (invalid token or wrong client ID)")
                return ValidationResult(
                    is_valid=False, 
            ...
            if validation_result.get("client_mismatch"):
                auth_logger.warning("OAuth token client ID mismatch - signaling for DCR re-registration")
                return ValidationResult(
                    is_valid=False,
            ...
            if token_info:
                auth_logger.info("Token validated for client: %s, scope: %s", 
                          token_info.get('audience'), token_info.get('scope'))
        else:
        ...
        if not workspace_user.is_valid:
            logger.warning("User %s has valid Google token but is not in our workspace", user_email)
            return ValidationResult(
                is_valid=False,
