...
from .token_validation import validate_oauth_token, ValidationResult
...
def initialize_oauth_config() -> None:
    ...
    try:    
        ...
        if not client_id:
            logger.warning("No client_id found in OAuth config - client ID validation will be disabled")
        elif not client_id.endswith('.googleusercontent.com'):
            logger.warning("OAuth client_id does not appear to be a valid Google client ID: %s", client_id)
        ...
        if dcr_client_id and dcr_client_secret:
            ...
            if dcr_client_id.endswith('.googleusercontent.com'):
                logger.info(f"DCR credentials loaded successfully for client: {dcr_client_id}")
            else:
                logger.warning(f"DCR client_id does not appear to be a valid Google client ID: {dcr_client_id}")
            
            # Validate client_secret length (Google client secrets are typically 24 chars)
            if len(dcr_client_secret) < 20:
                logger.warning("DCR client_secret appears to be too short - may be invalid")
            
            logger.info("Dynamic Client Registration (DCR) credentials: AVAILABLE")
        else:
            logger.warning("DCR credentials not found - Dynamic Client Registration will fail")
        ...
        logger.info(f"Successfully loaded Google OAuth configuration for client: {client_id or 'unknown'}")
        
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse GOOGLE_OAUTH as JSON: {e}")
        raise ValueError(f"Invalid JSON in GOOGLE_OAUTH environment variable: {e}")
...
def get_oauth_client_id() -> str | None:
    ...
    return _oauth_info.get('web', {}).get('client_id')


def initialize_mcp_servers_config() -> None:
    """Initialize MCP servers configuration (synchronous version for CLI)."""
    global _mcp_servers_config, _required_groups, _refresh_groups_interval_minutes, _refresh_users_interval_minutes

    # Get refresh intervals from environment
    _refresh_groups_interval_minutes = int(os.getenv('MCP_OAUTH_REFRESH_GROUPS_INTERVAL', '15'))
    _refresh_users_interval_minutes = int(os.getenv('MCP_OAUTH_REFRESH_USERS_INTERVAL', '60'))
    ...
    try:
        ...
        if _required_groups:
            ...
            logger.info(f"Found {len(mcp_servers)} servers with {len(_required_groups)} unique required groups: {sorted(_required_groups)}")
            logger.info(f"Scheduled refresh system will start on app startup (groups: {_refresh_groups_interval_minutes}min, users: {_refresh_users_interval_minutes}min)")
        else:
            logger.info(f"Loaded MCP servers config from {config_path} - no group requirements found")
...
async def initialize_mcp_servers_config_async() -> None:
    ...
    if _required_groups:
        try:
            ...
            logger.info(f"Started scheduled refresh system (groups: {_refresh_groups_interval_minutes}min, users: {_refresh_users_interval_minutes}min)")
        except Exception as e:
...
async def _start_scheduled_refresh() -> None:
    ...
    try:
        ...
        async def groups_refresh_loop():
            """Independent refresh loop for groups."""
            while True:
                try:
                    await asyncio.sleep(_refresh_groups_interval_minutes * 60)
                    logger.info("⏰ Groups refresh interval reached (%d minutes)", _refresh_groups_interval_minutes)
                    await _refresh_groups_and_users(google_wif_config, is_initial=False, 
                                                    refresh_groups=True, refresh_users=False)
        ...
        async def users_refresh_loop():
            """Independent refresh loop for users."""
            while True:
                try:
                    await asyncio.sleep(_refresh_users_interval_minutes * 60)
                    logger.info("⏰ Users refresh interval reached (%d minutes)", _refresh_users_interval_minutes)
                    await _refresh_groups_and_users(google_wif_config, is_initial=False, 
                                                    refresh_groups=False, refresh_users=True)
                except asyncio.CancelledError:
...
async def _refresh_groups_and_users(google_wif_config, is_initial: bool = False, 
                                    refresh_groups: bool = True, refresh_users: bool = True) -> None:
    ...
    try:
        if refresh_groups and _required_groups:
            logger.info(f"🔄 REFRESH: Attempting to refresh {len(_required_groups)} groups: {sorted(_required_groups)}")
            ...
            try:
                ...
                logger.debug(f"🔄 REFRESH: Calling internal method with groups: {group_list}")
                ...
                if hasattr(google_wif_config, 'refresh_groups'):
                    ...
                    logger.info(f"✅ REFRESH: Successfully refreshed {success_count} groups using refresh method")
                elif hasattr(google_wif_config, '_GoogleWIF__get_group_members'):
                    await google_wif_config._GoogleWIF__get_group_members(group_list, force_refresh=True)
                    success_count = len(group_list)
                    logger.info(f"✅ REFRESH: Successfully bulk-refreshed {success_count} groups using internal method")
                else:
                    raise AttributeError("Internal method not available")
                    
            except Exception as bulk_error:
                logger.warning(f"❌ REFRESH: Bulk refresh failed, falling back to individual group refresh: {bulk_error}")
                ...
                for group_name in _required_groups:
                    try:
                        ...
                        logger.debug(f"🔄 REFRESH: Clearing data and refreshing group: {group_name}")
                        ...
                        await google_wif_config.check_user_groups(dummy_email, [group_name])
                        ...
                        error_count += 1
                        logger.warning(f"❌ REFRESH: Failed to refresh group '{group_name}': {group_error}")
        ...
        if refresh_users:
            ...
            try:
                ...
                error_count += 1
                logger.warning(f"❌ REFRESH: Failed to refresh user cache: {user_error}")
        
        duration = time.time() - start_time
        
        # More detailed completion logging
        if is_initial:
            logger.info(f"🚀 {refresh_type} refresh completed in {duration:.2f}s: {success_count} items refreshed, {error_count} errors")
        else:
            logger.info(f"✅ {refresh_type} {action_str} refresh completed in {duration:.2f}s: {success_count} items refreshed, {error_count} errors")
        
        if error_count > 0:
            logger.warning(f"Some refresh operations failed - validation may be slower for affected items")
        
    except Exception as e:
        duration = time.time() - start_time
        logger.error(f"❌ {refresh_type} {action_str} refresh failed after {duration:.2f}s: {e}")
...
async def oauth_authorization_server(request: Request) -> JSONResponse:
    ...
    logger.info("OAuth discovery request from User-Agent: %s, ambiguous client: %s", 
               user_agent[:100] + "..." if len(user_agent) > 100 else user_agent, is_ambiguous_client)
...
async def refresh_status():
    ...
    status = {
        ...
        "refresh_task_running": _refresh_task is not None and not _refresh_task.done() if _refresh_task else False,
    ...
    if _refresh_task:
        status["refresh_task_status"] = "running" if not _refresh_task.done() else "completed"
...
async def dynamic_client_registration(request: Request) -> JSONResponse:
    ...
    logger.info("DCR client detection - client_name: '%s', user_agent: '%s'", 
               client_name, user_agent[:100] + "..." if len(user_agent) > 100 else user_agent)
    ...
    for uri in redirect_uris:
        ...
        # Special case: Accept localhost callbacks (we'll fix them in the auth proxy)
            ...
            try:
                port_part = uri.replace("http://localhost:", "").replace("/callback", "")
                port = int(port_part)
                if 1024 <= port <= 65535:
                    # Accept the localhost URL - our auth proxy will handle the forwarding
                    valid_redirects.append(uri)
                    logger.info("Accepted Claude Code localhost callback: %s (will be proxied)", uri)
                else:
                    logger.warning("Invalid redirect URI rejected (port out of range): %s", uri)
            except ValueError:
    ...
    registration_response["original_redirect_uri"] = valid_redirects[0]  # Claude Code's localhost
    registration_response["preferred_redirect_uri"] = proxy_callback  # Use our proxy for token exchange
        
    logger.info("Claude Code DCR: Configured for proxy-based OAuth flow with callback forwarding")
    
    # Log successful registration with endpoint decision
    logger.info("DCR successful - Client Type: %s, Endpoint Type: %s, Using Client ID: %s, Redirect URIs: %s", 
                "Standard",
                "proxy",
...
async def oauth_authorization_proxy(request: Request) -> Response:
    ...
    logger.info("Authorization proxy request received with parameters: %s", 
                {k: v if k not in ["client_id", "client_secret"] else "***" for k, v in params.items()})
    ...
    corrected_params["state"] = f"{original_state}|{session_id}" if original_state else session_id
    
    logger.info("Created auth forwarding session %s: %s -> %s", 
                session_id[:8], proxy_callback, redirect_uri)
    logger.info("Modified OAuth state: '%s' -> '%s'", 
                original_state, corrected_params["state"])
    ...
    if "access_type" in params:
        if params["access_type"] != "offline":
            ...
            logger.info("Changed access_type from '%s' to 'offline' for refresh token support", params["access_type"])
        else:
    ...
    if "prompt" in params and params["prompt"] != "none":
        ...
        logger.info("Removed prompt=none to enable refresh token (Google will show consent if needed)")
    else:
    ...
    logger.info("Proxying authorization request: %s -> %s", 
               redirect_uri, corrected_params["redirect_uri"])
    logger.info("Added missing scopes: %s", corrected_params["scope"])
    logger.info("OAuth parameters being sent to Google: access_type=%s, prompt=%s", 
...
async def oauth_callback_forwarding(request: Request) -> Response:
    ...
    logger.info("OAuth callback received with parameters: %s", 
                {k: v if k not in ["code", "state"] else "***" for k, v in query_params.items()})
    
    # OAuth error handling
    error = query_params.get("error")
    if error:
        logger.warning("OAuth callback received error: %s - %s", 
                      error, query_params.get("error_description", "No description"))
        return JSONResponse(
            status_code=400,
            content={
                "error": error,
                "error_description": query_params.get("error_description", "OAuth authorization failed")
            }
        )
    ...
    current_time = time.time()
    expired_sessions = [
        sid for sid, (_, timestamp) in _callback_forwarding.items()
    ...
    for expired_sid in expired_sessions:
        ...
        logger.debug("Cleaned up expired callback forwarding session: %s", expired_sid[:8])
    ...
    logger.info("Processing OAuth callback - state parameter: '%s', active sessions: %s", 
               oauth_state, list(_callback_forwarding.keys()))
    
    if oauth_state:
        if "|" in oauth_state:
            original_state, session_id = oauth_state.rsplit("|", 1)
            logger.info("Extracted session ID from compound state: original='%s', session_id='%s'", 
                       original_state, session_id[:8] + "..." if session_id else "None")
        else:
            ...
            if len(oauth_state) == 36 and oauth_state.count("-") == 4:
                ...
                logger.info("Using entire state as session ID: %s", session_id[:8] + "...")
            else:
                original_state = oauth_state
                logger.info("Treating state as original client state: %s", original_state)
    
    if not session_id:
        logger.warning("OAuth callback missing session correlation - no session ID in state parameter")
        return JSONResponse(
            status_code=400,
            content={"error": "missing_session", "error_description": "Cannot correlate OAuth callback to forwarding session"}
        )
    ...
    if session_id not in _callback_forwarding:
        ...
        return JSONResponse(
            status_code=404,
            content={"error": "session_not_found", "error_description": "Forwarding session not found or expired"}
        )
    ...
    if current_time - timestamp > _callback_forwarding_ttl:
        ...
        return JSONResponse(
            status_code=410,
            content={"error": "session_expired", "error_description": "Forwarding session has expired"}
        )
    ...
    logger.info("Redirecting OAuth callback from session %s to %s via browser", session_id[:8], localhost_callback)
...
async def oauth_token_proxy(request: Request) -> Response:
    ...
    logger.info("Token exchange request received from %s to %s", 
    ...
    if request.headers.get("content-type", "").startswith("application/x-www-form-urlencoded"):
        ...
        params = dict(await request.json()) if request.headers.get("content-type") == "application/json" else {}
    
    logger.info("Token exchange proxy request received with grant_type: %s", params.get("grant_type", "unknown"))
    logger.debug("Original token exchange parameters: %s", 
                {k: v if k not in ["client_secret", "code", "code_verifier"] else "***" for k, v in params.items()})
    ...
    logger.debug("🔍 AUTH DEBUG - Authorization header type: %s", 
                auth_header.split(" ")[0] if auth_header else "none")
    logger.debug("🔍 AUTH DEBUG - Content-Type: %s", request.headers.get("content-type", "none"))
    logger.debug("🔍 AUTH DEBUG - Request headers: %s", 
    ...
    if auth_header and auth_header.startswith("Basic "):
        try:
            ...
            if ":" in decoded:
                ...
                logger.warning("🔍 AUTH DEBUG - Invalid Basic Auth format (no colon separator)")
        except Exception as e:
    ...
    logger.info("🔍 AUTH DEBUG - client_id in body: %s", "present" if client_id_body else "missing")
    logger.info("🔍 AUTH DEBUG - client_secret in body: %s", "present" if client_secret_body else "missing")
    logger.info("🔍 AUTH DEBUG - client_id in header: %s", "present" if client_id_header else "missing")
    logger.info("🔍 AUTH DEBUG - client_secret in header: %s", "present" if client_secret_header else "missing")
    logger.info("🔍 AUTH DEBUG - Final client_id: %s (from %s)", 
               "present" if client_id else "missing",
               "header" if client_id_header else "body" if client_id_body else "none")
    logger.info("🔍 AUTH DEBUG - Final client_secret: %s (from %s)", 
    ...
    if not client_id or not client_secret:
        logger.error("Token exchange failed - missing client credentials: client_id=%s, client_secret=%s", 
                    "present" if client_id else "missing", 
                    "present" if client_secret else "missing")
        return JSONResponse(
            status_code=401,
    ...
    if dcr_client_id and client_id != dcr_client_id:
        logger.error("Token exchange failed - invalid client_id: expected %s, got %s", 
    ...
    if params.get("grant_type") == "authorization_code":
        ...
        if original_redirect_uri:
            ...
            logger.info("Token exchange: Fixed redirect_uri %s -> %s", original_redirect_uri, proxy_callback)
    
    # Forward the corrected token exchange request to Google
    logger.info("Forwarding %s token exchange request to Google OAuth", params.get("grant_type", "unknown"))
    logger.debug("Token exchange parameters: %s", 
                {k: v if k not in ["client_secret", "code", "refresh_token"] else "***" for k, v in corrected_params.items()})
    ...
    try:
        ...
        logger.info("Token exchange response from Google: status=%d", response.status_code)
        
        # Log the exact response we got from Google
        logger.info("🔍 Raw Google response - Status: %d, Headers: %s", 
                   response.status_code, dict(response.headers))
        logger.info("🔍 Raw Google response - Content length: %d bytes", len(response.content))
        ...
        try:
            ...
            if response.headers.get("content-encoding") == "gzip":
                ...
                logger.info("🔍 Google response is gzipped - decompressed size: %d bytes", len(decompressed_content))
                token_data = json.loads(decompressed_content.decode('utf-8'))
            else:
                token_data = response.json()
                
            logger.info("🔍 Google token response structure: %s", 
        ...
        if response.status_code == 200:
            try:
                ...
                if "refresh_token" in token_response:
                    ...
                    logger.warning("⚠️ No refresh_token received from Google - this may cause token expiration issues")
        ...
        if response.status_code != 200:
            try:
                ...
                logger.error("Token exchange failed - Google response body: %s", response.text[:500])
        ...
        if content_encoding == 'gzip':
            ...
            try:
                ...
                logger.info("✅ Content already decompressed by httpx, removing gzip header")
                response_headers.pop('content-encoding', None)
                response_headers['content-length'] = str(len(response.content))
                
                logger.info("🔍 Token data from Google: %s", {
                    k: f"{str(v)[:20]}..." if k in ["access_token", "refresh_token", "id_token"] else v 
                ...
                try:
                    ...
                    logger.info("✅ Decompressed response: %d -> %d bytes", len(response.content), len(response_content))
                except Exception as e:
                    logger.error("❌ Failed to decompress gzipped response: %s", e)
                    # Fall back to original response
                    pass
        
        logger.info("✅ Sending response to client: status=%d, content_length=%d bytes", 
                   response.status_code, len(response_content))
        logger.info("🔍 Final response - Content-Encoding: %s, Content-Length: %s", 
...
async def validate_user_with_groups(request: Request):
    ...
    try:
        ...
        if groups_to_check:
            try:
                group_memberships = await google_wif_config.check_user_groups(user_email, groups_to_check)
                logger.info("User %s group membership check: %s", user_email, group_memberships)
            except Exception as group_error:
                logger.warning("Failed to check group memberships for user %s: %s", user_email, group_error)
