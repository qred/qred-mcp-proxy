    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    "Google OAuth token validated for user: %s", user_info.get("email")
                )
                return user_info
            else:
                logger.warning(
                    "Google OAuth token validation failed: HTTP %d",
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                token_audience = token_info.get(
                    "audience"
                )  # 'audience' contains the client_id

                if token_audience == expected_client_id:
                    logger.info(
                        "Token client ID validated: %s for user %s",
                        expected_client_id,
                        user_info.get("email"),
                    )
                ...
                logger.warning(
                    "Google token introspection failed: HTTP %d", response.status_code
                )
...
def check_req_env_vars(required_env_vars: list[str]) -> None:
