...
from .aggregated_proxy_server import create_aggregated_proxy_server, set_user_context, set_user_context
from .oauth_client import validate_request_user, ValidationResult
from ..utils.logger import logger


def get_base_url(request: Request) -> str:
    """
    Get base URL for the request, handling HTTPS enforcement for configured domains.
    """
    ...
    scheme = "https" if should_force_https else request.url.scheme
...
async def _validate_user_request(request: Request) -> tuple[bool, str | None, str | None, str | None]:
    """
    ...
    if result.is_valid:
        logger.info("OAuth validation: User %s authenticated successfully", result.user_email or "unknown")
        return True, result.user_email, None, token
    else:
        logger.warning("OAuth validation: Authentication failed for user - %s", result.error or "unknown error")
...
class MCPServerSettings:
    ...
    aggregated_mode: bool = False  # Whether to run in aggregated mode (single server with all tools)
    ...
    *,
    ...
    async def handle_sse_instance(request: Request) -> Response:
        ...
        if google_auth_required:
            user_valid, user_email, error_code, access_token = await _validate_user_request(request)
            if not user_valid:
                ...
                if error_code == "missing_token":
                    ...
                    auth_header = f'Bearer realm="MCP Server"'
                    error_desc = "OAuth Bearer token required for MCP access"
                elif error_code == "invalid_client":
                    # Token issued for wrong client - signal DCR re-registration
                    auth_header = f'Bearer realm="MCP Server", error="invalid_client", error_description="Token was issued for a different client ID"'
                    error_desc = "OAuth token client ID mismatch - DCR re-registration required"
                else:
                    # Invalid token or other errors - include error details  
                    auth_header = f'Bearer realm="MCP Server", error="invalid_token", error_description="Token validation failed"'
                    error_desc = "OAuth Bearer token is invalid or user not in Google Workspace"
    ...
    async def handle_streamable_http_instance(scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if is_mcp_request:
            ...
            has_auth = auth_header.startswith("Bearer ") if auth_header != "NONE" else False
            
            if method == "POST" and has_auth:
                # This is likely a new authenticated MCP session
                logger.info("MCP session: %s from %s (%s)", method, client_info, 
                           user_agent)
            
            # Detailed debugging only when debug is enabled
            logger.debug("🔍 REQUEST: %s %s from %s | User-Agent: %s | Auth: %s", 
                                 method, path, client_info, user_agent, 
            ...
            if user_email:
                ...
                if "/status" not in path:
                    ...
                    if method == "POST" and path.startswith("/mcp"):
                        ...
                        logger.debug("MCP session from %s: %s %s", user_email or "anonymous", method, path)
                    
                    # Log tool-specific activities with debug details when enabled
                    logger.debug("User activity - User: %s, Action: http_request, Details: %s", 
                                        user_email, {"endpoint": path, "method": method, "remote_addr": remote_addr})
                else:
                    # Log status checks at debug level only
                    logger.debug("Health check request from user %s: %s %s", 
                                        user_email, method, path)
...
async def run_mcp_server(
    ...
    _global_status["authentication"]["google_auth_required"] = mcp_settings.google_auth_required

    all_routes: list[BaseRoute] = [
        Route("/status", endpoint=_handle_status),  # Global status endpoint - always public
    ]
    ...
    async with contextlib.AsyncExitStack() as stack:
        ...
        if mcp_settings.aggregated_mode:
            ...
            logger.info("Running in AGGREGATED MODE - combining all backends into single MCP server")
            ...
            stdio_backends = {name: params for name, params in all_backends.items() 
                            if isinstance(params, StdioServerParameters)}
            http_backends = {name: params for name, params in all_backends.items() 
                           if isinstance(params, HttpServerParameters)}
            
            if stdio_backends:
                logger.info("Applying startup mitigations for %d STDIO backends...", len(stdio_backends))
                mitigation_start = time.time()
                
                try:
                    mitigated_stdio_backends = await StartupMitigation.apply_startup_mitigations(stdio_backends)
                    mitigation_duration = time.time() - mitigation_start
                    logger.info("Startup mitigations completed in %.2fs", mitigation_duration)
                    
                    # Record mitigation for monitoring
                    for backend_name in stdio_backends:
                        if backend_name in mitigated_stdio_backends and mitigated_stdio_backends[backend_name] != stdio_backends[backend_name]:
                    ...
                    mitigation_duration = time.time() - mitigation_start
                    logger.warning("Startup mitigations failed after %.2fs: %s", mitigation_duration, e)
            ...
            logger.info("Running pre-startup health checks on %d backends...", len(all_backends))
            health_results = await validate_backend_processes(all_backends, max_concurrent=3)
            
            # Record health check results
            for name, (is_healthy, error, duration) in health_results.items():
                startup_monitor.record_backend_health_check(name, is_healthy, error, duration)
            
            healthy_backends = {name: params for name, params in all_backends.items() 
                              if health_results.get(name, (False, None, 0.0))[0]}
            failed_backends = {name: error for name, (is_healthy, error, duration) in health_results.items() 
                             if not is_healthy}
            
            if failed_backends:
                logger.warning("Some backends failed health checks and will be excluded:")
                for name, error in failed_backends.items():
                    logger.warning("  - %s: %s", name, error)
                    startup_monitor.record_backend_skipped(name, f"Health check failed: {error}")
            ...
            logger.info("Using %d healthy backends: %s", len(healthy_backends), ", ".join(healthy_backends.keys()))
            
            # Phase 3: Session creation
            startup_monitor.start_phase("SESSION_CREATION")
            logger.info("Using long-lived sessions approach - keep-alive tasks will be created")
            ...
            for name, params in healthy_backends.items():
                ...
                try:
                    if isinstance(params, HttpServerParameters):
                        logger.info("Setting up backend '%s': HTTP %s", name, params.url)
                        http_streams = await stack.enter_async_context(
                            streamablehttp_client(url=params.url, headers=params.headers)
                        )
                        session = await stack.enter_async_context(ClientSession(*http_streams[:2]))
                    else:
                        logger.info("Setting up backend '%s': %s %s", name, params.command, " ".join(params.args))
                    ...
                    startup_monitor.record_session_result(name, True, None, session_duration)
                    
                except Exception as e:
                    session_duration = time.time() - session_start
                    startup_monitor.record_session_result(name, False, str(e), session_duration)
                    logger.error("Failed to create session for backend '%s': %s", name, e)
            ...
            for name in failed_backends:
                _global_status["server_instances"][name] = f"failed health check: {failed_backends[name]}"
            ...
            logger.info("Aggregated server configured with %d healthy backends", len(healthy_backends))
        
        else:
            # Original individual server mode
            logger.info("Running in INDIVIDUAL MODE - separate endpoints for each backend")
        ...
        # Setup default server if configured
            ...
            for name, params in named_server_params.items():
                if isinstance(params, HttpServerParameters):
                    ...
                    session_named = await stack.enter_async_context(ClientSession(*http_streams[:2]))
        ...
        config = uvicorn.Config(
            ...
            access_log=mcp_settings.log_level == "DEBUG",  # Only show access logs in DEBUG mode
        ...
        if mcp_settings.aggregated_mode:
            ...
            # Add named servers
            mcp_urls.extend([f"{base_url}/servers/{name}/mcp" for name in named_server_params])
        ...
        if mcp_urls:
            ...
            for url in mcp_urls:
                auth_note = " (Google auth required)" if mcp_settings.google_auth_required else ""
