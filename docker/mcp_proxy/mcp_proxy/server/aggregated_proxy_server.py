    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                # groups is a dict like {"group1@example.com": true, "group2@example.com": false}
                for required_group in required_groups:
                    if user_groups_dict.get(required_group, False):
                        logger.debug("User %s authorized for group %s", user_email, required_group)
                        return True, ""
                
                # Get list of user's actual groups for logging
                user_groups = [group for group, is_member in user_groups_dict.items() if is_member]
                logger.warning("User %s not in required groups %s (user groups: %s)", 
                             user_email, required_groups, user_groups)
                return False, f"Access denied: You must be a member of one of these groups: {', '.join(required_groups)}"
                ...
                logger.error("OAuth validation failed with status %d: %s", response.status_code, response.text)
...
async def _keep_backend_alive(backend_name: str, session: ClientSession, capabilities: types.ServerCapabilities) -> None:
    ...
    keepalive_logger.info("KEEP-ALIVE START: Backend '%s' keep-alive task started", backend_name)
    ...
    while True:
        ...
        try:
            ...
            if capabilities.tools:
                keepalive_logger.debug("KEEP-ALIVE PING %d: Calling list_tools for backend '%s'", ping_count, backend_name)
                logger.debug("Keep-alive: calling list_tools for backend '%s'", backend_name)
                await session.list_tools()
                ping_duration = time.time() - ping_start
                keepalive_logger.info("KEEP-ALIVE SUCCESS: Backend '%s' ping %d completed in %.2fs (list_tools)", 
                                    backend_name, ping_count, ping_duration)
                logger.debug("Keep-alive: list_tools successful for backend '%s'", backend_name)
            elif capabilities.resources:
                keepalive_logger.debug("KEEP-ALIVE PING %d: Calling list_resources for backend '%s'", ping_count, backend_name)
                logger.debug("Keep-alive: calling list_resources for backend '%s'", backend_name)
                await session.list_resources()
                ping_duration = time.time() - ping_start
                keepalive_logger.info("KEEP-ALIVE SUCCESS: Backend '%s' ping %d completed in %.2fs (list_resources)", 
                                    backend_name, ping_count, ping_duration)
                logger.debug("Keep-alive: list_resources successful for backend '%s'", backend_name)
            elif capabilities.prompts:
                keepalive_logger.debug("KEEP-ALIVE PING %d: Calling list_prompts for backend '%s'", ping_count, backend_name)
                logger.debug("Keep-alive: calling list_prompts for backend '%s'", backend_name)
                await session.list_prompts()
                ping_duration = time.time() - ping_start
                keepalive_logger.info("KEEP-ALIVE SUCCESS: Backend '%s' ping %d completed in %.2fs (list_prompts)", 
                                    backend_name, ping_count, ping_duration)
                logger.debug("Keep-alive: list_prompts successful for backend '%s'", backend_name)
            else:
                keepalive_logger.warning("KEEP-ALIVE WARNING: Backend '%s' has no capabilities for keep-alive - connection may timeout", backend_name)
                logger.warning("Backend '%s' has no capabilities for keep-alive - connection may timeout", backend_name)
                
        except asyncio.CancelledError:
            runtime = time.time() - start_time
            keepalive_logger.info("KEEP-ALIVE STOP: Backend '%s' task cancelled after %.1f minutes (%d pings)", 
                                backend_name, runtime / 60, ping_count)
            logger.info("Keep-alive task cancelled for backend '%s'", backend_name)
            break
        except Exception as e:
            ping_duration = time.time() - ping_start if ping_start else 0
            keepalive_logger.error("KEEP-ALIVE ERROR: Backend '%s' ping %d failed after %.2fs: %s", 
                                 backend_name, ping_count, ping_duration, e)
            logger.error("Keep-alive failed for backend '%s': %s", backend_name, e)
            logger.error("Backend '%s' connection may have been lost - this is expected after long inactivity", backend_name)
...
async def cleanup_keep_alive_tasks() -> None:
    ...
    if _global_keep_alive_tasks:
        keepalive_logger.info("KEEP-ALIVE CLEANUP: Starting cleanup of %d keep-alive tasks", len(_global_keep_alive_tasks))
        ...
        keepalive_logger.info("KEEP-ALIVE CLEANUP: Cancelled %d running tasks, waiting for completion", cancelled_count)
        
        # Wait for all tasks to finish cancelling
        await asyncio.gather(*_global_keep_alive_tasks, return_exceptions=True)
        _global_keep_alive_tasks.clear()
        
        keepalive_logger.info("KEEP-ALIVE CLEANUP: All keep-alive tasks cleaned up successfully")
        logger.info("All keep-alive tasks cleaned up")


def _backend_needs_keepalive(backend_name: str, backend_sessions: Mapping[str, ClientSession], backend_params: Mapping[str, Any] | None = None) -> bool:
    ...
    if backend_params and backend_name in backend_params:
        ...
        if hasattr(params, 'command'):
            ...
            if any(indicator in command for indicator in ['mcp-remote', 'http://', 'https://', 'wss://', 'ws://']):
                return True
                
            # Check args for remote indicators
            if hasattr(params, 'args') and params.args:
                args_str = ' '.join(str(arg) for arg in params.args).lower()
                if any(indicator in args_str for indicator in ['http://', 'https://', 'sse', 'websocket']):
                    return True
...
def log_user_activity(action: str, details: dict) -> None:
    ...
    if user_email:
        logger.info("User activity - User: %s, Action: %s, Details: %s", 
...
async def _build_tool_exclusion_cache(
    ...
    for backend_name, patterns in excluded_tools.items():
        ...
        if not backend_capabilities.get(backend_name, types.ServerCapabilities()).tools:
            logger.debug("Backend '%s' has no tools capability, skipping exclusion cache build", backend_name)
            continue
        ...
        try:
            logger.debug("Building tool exclusion cache for backend '%s' with patterns: %s", backend_name, patterns)
            ...
            if tools_result.tools:
                for tool in tools_result.tools:
                    ...
                    for pattern in patterns:
                        ...
                        if pattern == tool_name:
                            should_exclude = True
                        elif pattern.endswith('*') and tool_name.startswith(pattern[:-1]):
                            should_exclude = True
                        elif pattern.startswith('*') and tool_name.endswith(pattern[1:]):
                            should_exclude = True
                        elif '*' in pattern:
                            import fnmatch
                ...
                if excluded_for_backend:
                    logger.info("🚫 Tool filtering: Excluding %d tools from backend '%s': %s", 
                               len(excluded_for_backend), backend_name, 
                               ', '.join(sorted(excluded_for_backend)[:5]) + 
                               (f' (and {len(excluded_for_backend) - 5} more)' if len(excluded_for_backend) > 5 else ''))
                else:
                    logger.debug("No tools excluded for backend '%s' (patterns didn't match any tools)", backend_name)
            else:
                logger.debug("Backend '%s' returned no tools for exclusion cache", backend_name)
                _excluded_tools_cache[backend_name] = set()
                
        except Exception as e:
            logger.warning("Failed to build tool exclusion cache for backend '%s': %s", backend_name, e)
            _excluded_tools_cache[backend_name] = set()
    
    if total_excluded > 0:
        logger.info("Tool exclusion cache built: %d tools excluded across %d backends", 
                   total_excluded, len([b for b in _excluded_tools_cache.values() if b]))
    else:
...
async def _setup_proxy_logging(app: server.Server[object]) -> None:
    ...
    async def _set_proxy_logging_level(req: types.SetLevelRequest) -> types.ServerResult:
...
async def create_aggregated_proxy_server(
    ...
    server_name: str = "Aggregated MCP Server",
    ...
    """
    logger.info("Creating aggregated MCP server with %d backends: %s", 
                len(backend_sessions), ", ".join(backend_sessions.keys()))
    ...
    logger.info("Initializing MCP sessions for %d pre-validated backends", len(backend_sessions))
    ...
    for name, session in backend_sessions.items():
        ...
        for attempt in range(max_retries + 1):  # 0, 1, 2, 3, 4 (5 total attempts)
            ...
            try:
                if attempt > 0:
                    logger.info("Initializing backend '%s'... (attempt %d/%d)", name, attempt + 1, max_retries + 1)
                ...
                backend_params_for_name = backend_params.get(name) if backend_params else None
                if backend_params_for_name and hasattr(backend_params_for_name, 'args') and backend_params_for_name.args:
                    args_str = ' '.join(str(arg) for arg in backend_params_for_name.args)
                    if 'mcp-remote' in args_str and '127.0.0.1' in args_str:
                        startup_delay = 1.0 if attempt == 0 else 0.5  # Longer delay on first attempt
                        logger.debug("Backend '%s' uses local mcp-remote - adding %.1fs startup delay", name, startup_delay)
                        await asyncio.sleep(startup_delay)
                
                # Add timeout for backend initialization to prevent hanging
                session_init_start = time.time()
                try:
                    logger.debug("Starting MCP session.initialize() for backend '%s'", name)
                    response = await asyncio.wait_for(session.initialize(), timeout=30.0)
                    init_duration = time.time() - session_init_start
                    logger.debug("Backend '%s' session.initialize() completed in %.2fs", name, init_duration)
                    
                    backend_capabilities[name] = response.capabilities
                    backend_info[name] = response.serverInfo
                    
                    if attempt > 0:
                        logger.info("Backend '%s' initialized successfully on attempt %d: %s (%.2fs)", 
                                   name, attempt + 1, response.serverInfo.name, init_duration)
                    else:
                        logger.info("Backend '%s' initialized successfully: %s (%.2fs)", name, response.serverInfo.name, init_duration)
                    
                    logger.info("Backend '%s' capabilities: tools=%s, resources=%s, prompts=%s, logging=%s", 
                                name, 
                    ...
                    if attempt < max_retries:
                        logger.warning("Backend '%s' initialization timed out after 30 seconds (attempt %d/%d) - retrying...", 
                                      name, attempt + 1, max_retries + 1)
                        continue
                    else:
                        logger.error("Backend '%s' initialization timed out after 30 seconds (final attempt)", name)
                        logger.error("Backend timeout for '%s' - continuing with other backends", name)
                        logger.error("This could indicate network issues or slow remote endpoints")
                        break
                ...
                is_retriable_error = (
                    ...
                    error_type in ["ClosedResourceError", "ConnectionError", "TimeoutError"] or
                    # Check string content (for message-based errors)
                    any(error_pattern in error_str for error_pattern in [
                        "SSE error",
                        "Connection refused", 
                        "Connection reset",
                        "BrokenPipeError",
                ...
                is_process_error = any(error_pattern in error_str for error_pattern in [
                    "SyntaxError",
                    "Module not found", 
                    "Cannot find module",
                    "Command not found",
                ...
                if attempt < max_retries and (is_retriable_error or (is_process_error and not process_recreation_attempted)):
                    # Normal retry logic
                    if not process_recreation_attempted or not is_process_error:
                        logger.warning("Backend '%s' initialization failed with %s after %.2fs (attempt %d/%d) - retrying in %.1fs...", 
                                      name, error_type, attempt_duration, attempt + 1, max_retries + 1, retry_delay)
                    ...
                    if attempt == max_retries:
                        logger.error("Backend '%s' initialization failed after %d attempts with %s (%.2fs)", 
                                    name, max_retries + 1, error_type, attempt_duration)
                    else:
                        logger.error("Backend '%s' initialization failed with non-retriable error: %s (%.2fs)", name, error_type, attempt_duration)
                    
                    logger.error("Failed to initialize backend '%s': %s", name, e)
                    if "SSE error" in error_str or "timeout" in error_str.lower():
                        logger.error("Backend '%s' appears to use SSE or remote connections - this can be unstable", name)
                        logger.error("Consider using local backends or contact the service provider about connection stability")
                    elif "ClosedResourceError" in error_str:
                        logger.error("Backend '%s' has connection stream issues - this may be due to rapid reconnection attempts", name)
                        logger.error("The retry mechanism should help, but persistent failures may indicate backend instability")
                    elif is_process_error:
                        logger.error("Backend '%s' has process-level issues that may require environment fixes", name)
                        logger.error("Check that all dependencies are installed and accessible")
                    
                    logger.exception("Full traceback for backend '%s' initialization failure:", name)
                    break    # Check if we have at least one working backend
    if not backend_capabilities:
        logger.error("No backends successfully initialized! Cannot create aggregated server.")
        raise RuntimeError("All backend initializations failed")
    
    successful_backends = list(backend_capabilities.keys())
    logger.info("Successfully initialized %d backend(s): %s", len(successful_backends), ", ".join(successful_backends))
    
    failed_backends = set(backend_sessions.keys()) - set(successful_backends)
    if failed_backends:
        logger.warning("Failed to initialize backend(s): %s", ", ".join(failed_backends))
    
    # Start keep-alive tasks for backends that need them (remote/SSE backends)
    keep_alive_tasks = []
    for backend_name, session in backend_sessions.items():
        if backend_name in backend_capabilities and _backend_needs_keepalive(backend_name, backend_sessions, backend_params):
            # This backend uses remote connections and needs keep-alive to prevent timeouts
            keepalive_logger.info("KEEP-ALIVE INIT: Starting keep-alive task for backend '%s' (remote/SSE connection detected)", backend_name)
            keep_alive_task = asyncio.create_task(
                _keep_backend_alive(backend_name, session, backend_capabilities[backend_name])
            )
            keep_alive_tasks.append(keep_alive_task)
            logger.info("Started keep-alive task for backend '%s' (remote/SSE connection detected)", backend_name)
        else:
            keepalive_logger.debug("KEEP-ALIVE SKIP: Backend '%s' does not need keep-alive (local connection)", backend_name)
    
    if keep_alive_tasks:
        keepalive_logger.info("KEEP-ALIVE SUMMARY: Started %d keep-alive tasks for remote backends", len(keep_alive_tasks))
    else:
        keepalive_logger.info("KEEP-ALIVE SUMMARY: No keep-alive tasks needed (all backends are local)")
    ...
    if any(caps.logging for caps in backend_capabilities.values()):
        logger.info("Setting up aggregated logging capability (backends support logging)...")
        logger.info("Backends with logging support: %s", [name for name, caps in backend_capabilities.items() if caps.logging])
        await _setup_aggregated_logging(app, backend_sessions, backend_capabilities)
        logger.info("Aggregated logging setup complete. SetLevel handler registered: %s", 
                   types.SetLevelRequest in app.request_handlers)
    else:
        logger.info("Setting up proxy logging capability (no backends support logging)...")
        await _setup_proxy_logging(app)
        logger.info("Proxy logging setup complete. SetLevel handler registered: %s", 
                   types.SetLevelRequest in app.request_handlers)
    
    # Aggregate tools
    if any(caps.tools for caps in backend_capabilities.values()):
        logger.debug("Setting up aggregated tools...")
        await _setup_aggregated_tools(app, backend_sessions, backend_capabilities, excluded_tools, required_groups)
    
    # Aggregate resources
    if any(caps.resources for caps in backend_capabilities.values()):
        logger.debug("Setting up aggregated resources...")
        await _setup_aggregated_resources(app, backend_sessions, backend_capabilities, required_groups)
    
    # Aggregate prompts
    if any(caps.prompts for caps in backend_capabilities.values()):
        logger.debug("Setting up aggregated prompts...")
        await _setup_aggregated_prompts(app, backend_sessions, backend_capabilities, required_groups)
    
    logger.info("Aggregated MCP server ready with backends: %s", 
    ...
    try:
        ...
        logger.info("Registered request handlers: %s", list(app.request_handlers.keys()))
        logger.info("SetLevelRequest handler registered: %s", types.SetLevelRequest in app.request_handlers)
        logger.info("Server will advertise logging capability: %s", bool(app.request_handlers.get(types.SetLevelRequest)))
    except Exception as e:
...
async def _setup_aggregated_tools(
    ...
    await _build_tool_exclusion_cache(backend_sessions, backend_capabilities, excluded_tools)
    ...
    async def _list_tools(_: t.Any) -> types.ServerResult:  # noqa: ANN401
        ...
        log_user_activity("list_tools", {
            "backends": list(backend_sessions.keys())
        })
        ...
        for backend_name, session in backend_sessions.items():
            ...
            if backend_required_groups:
                ...
                if not access_token:
                    logger.debug("Backend '%s' requires groups %s but no access token available - skipping", 
                               backend_name, backend_required_groups)
                    continue
                
                # Validate user groups
                is_authorized, error_msg = await _validate_user_groups(access_token, backend_required_groups)
                if not is_authorized:
                    user_email = get_user_context()
                    logger.info("🚫 Access control: User %s not authorized for backend '%s' tools - %s", 
                               user_email or 'unknown', backend_name, error_msg)
                    continue
                
                logger.debug("✅ Access control: User authorized for backend '%s' (requires groups: %s)", 
                           backend_name, backend_required_groups)
                
            try:
                logger.debug("Listing tools from backend '%s'...", backend_name)
                tools_result = await session.list_tools()
                logger.debug("Backend '%s' returned %d tools", backend_name, 
                ...
                logger.error("Failed to list tools from backend '%s': %s", backend_name, e)
                logger.exception("Full traceback for backend '%s' tools listing:", backend_name)
                continue
    ...
    async def _call_tool(req: types.CallToolRequest) -> types.ServerResult:
        ...
        if "_" not in tool_name:
            raise ValueError(f"Invalid tool name format: {tool_name}. Expected: backend_toolname")
        ...
        if backend_required_groups:
            ...
            if not is_authorized:
                user_email = get_user_context()
                logger.warning("🚫 Tool execution blocked: User %s not in required groups for backend '%s' (tool: %s) - %s", 
                             user_email or 'unknown', backend_name, actual_tool_name, error_msg)
                raise McpError(
                    types.ErrorData(
                        code=types.INVALID_REQUEST,
                        message=error_msg
            ...
            logger.debug("User authorized for tool call on backend '%s' (groups: %s)", 
                       backend_name, backend_required_groups)
        ...
        if _should_exclude_tool(backend_name, actual_tool_name):
            ...
            logger.warning("Blocked attempt to call excluded tool '%s' from backend '%s'. Reason: %s", 
                         actual_tool_name, backend_name, error_msg)
        
        try:
            logger.debug("Calling tool '%s' on backend '%s'", actual_tool_name, backend_name)
            ...
            if user_email:
                ...
                if should_override or "user" not in arguments:
                    ...
                    logger.debug("Automatically %s authenticated user '%s' for backend '%s' tool '%s'", 
                               injection_type, user_email, backend_name, actual_tool_name)
                else:
                    logger.debug("User parameter already provided for backend '%s' tool '%s', not overriding", 
                               backend_name, actual_tool_name)
            elif not user_email and backend_name in BACKENDS_REQUIRING_USER:
                logger.warning("No authenticated user context available for backend '%s' that requires user injection", backend_name)
            
            # Log user tool activity
            log_user_activity("tool_call", {
                "tool_name": actual_tool_name,
                "backend": backend_name,
                "namespaced_tool": tool_name,
                "arguments": {k: v for k, v in arguments.items() if k != "user"},  # Don't log sensitive user info
            ...
            logger.error("Tool call failed for '%s' on backend '%s': %s", 
                        actual_tool_name, backend_name, e)
            
            # Log failed tool activity
            log_user_activity("tool_call_failed", {
                "tool_name": actual_tool_name,
                "backend": backend_name,
                "namespaced_tool": tool_name,
                "error": str(e)
...
async def _setup_aggregated_resources(
    ...
    async def _list_resources(_: t.Any) -> types.ServerResult:  # noqa: ANN401
        ...
        log_user_activity("list_resources", {
            "backends": list(backend_sessions.keys())
        })
        ...
        for backend_name, session in backend_sessions.items():
            ...
            if backend_required_groups:
                ...
                if not is_authorized:
                    user_email = get_user_context()
                    logger.info("🚫 Access control: User %s not authorized for backend '%s' resources - %s", 
                               user_email or 'unknown', backend_name, error_msg)
                    continue
                
                logger.debug("✅ Access control: User authorized for backend '%s' resources (requires groups: %s)", 
                           backend_name, backend_required_groups)
                
            try:
                logger.debug("Listing resources from backend '%s'...", backend_name)
                resources_result = await session.list_resources()
                logger.debug("Backend '%s' returned %d resources", backend_name, 
                           len(resources_result.resources) if resources_result.resources else 0)
                ...
                if resources_result.resources:
                    ...
                    for resource in resources_result.resources:
                        ...
                        logger.debug("Added namespaced resource: %s", namespaced_resource.uri)
                        
            except Exception as e:
                logger.error("Failed to list resources from backend '%s': %s", backend_name, e)
                continue
    ...
    async def _read_resource(req: types.ReadResourceRequest) -> types.ServerResult:
        ...
        if "://" not in uri_str:
            raise ValueError(f"Invalid resource URI format: {uri_str}. Expected: backend://original_uri")
        ...
        if backend_required_groups:
            access_token = get_user_access_token()
            if not access_token:
                logger.warning("Backend '%s' requires groups %s but no access token available", 
                             backend_name, backend_required_groups)
                raise ValueError(f"Access denied: Backend '{backend_name}' requires authentication")
            
            # Validate user groups
            is_authorized, error_msg = await _validate_user_groups(access_token, backend_required_groups)
            if not is_authorized:
                user_email = get_user_context()
                logger.warning("🚫 Resource access blocked: User %s not in required groups for backend '%s' (resource: %s) - %s", 
                             user_email or 'unknown', backend_name, original_uri, error_msg)
                raise ValueError(f"Access denied: {error_msg}")
            
            logger.debug("User authorized for backend '%s' (groups: %s)", 
                       backend_name, backend_required_groups)
        ...
        try:
            ...
            logger.error("Resource read failed for '%s' on backend '%s': %s", 
                        original_uri, backend_name, e)
            raise
...
async def _setup_aggregated_prompts(
    ...
    async def _list_prompts(_: t.Any) -> types.ServerResult:  # noqa: ANN401
        ...
        log_user_activity("list_prompts", {
            "backends": list(backend_sessions.keys())
        })
        ...
        for backend_name, session in backend_sessions.items():
            ...
            if backend_required_groups:
                ...
                if not is_authorized:
                    user_email = get_user_context()
                    logger.info("🚫 Access control: User %s not authorized for backend '%s' prompts - %s", 
                               user_email or 'unknown', backend_name, error_msg)
                    continue
                
                logger.debug("✅ Access control: User authorized for backend '%s' prompts (requires groups: %s)", 
                           backend_name, backend_required_groups)
                
            try:
                logger.debug("Listing prompts from backend '%s'...", backend_name)
                prompts_result = await session.list_prompts()
                logger.debug("Backend '%s' returned %d prompts", backend_name, 
                ...
                if prompts_result.prompts:
                    ...
                    for prompt in prompts_result.prompts:
                        namespaced_prompt = types.Prompt(
                            ...
                            description=f"[{backend_name.upper()}] {prompt.description}" if prompt.description else None,
                            arguments=prompt.arguments
                        )
                        all_prompts.append(namespaced_prompt)
                        logger.debug("Added namespaced prompt: %s", namespaced_prompt.name)
                        
            except Exception as e:
                logger.error("Failed to list prompts from backend '%s': %s", backend_name, e)
                continue
    ...
    async def _get_prompt(req: types.GetPromptRequest) -> types.ServerResult:
        ...
        if "_" not in prompt_name:
            raise ValueError(f"Invalid prompt name format: {prompt_name}. Expected: backend_promptname")
        ...
        if backend_required_groups:
            ...
            if not is_authorized:
                user_email = get_user_context()
                logger.warning("🚫 Prompt execution blocked: User %s not in required groups for backend '%s' (prompt: %s) - %s", 
                             user_email or 'unknown', backend_name, actual_prompt_name, error_msg)
                raise ValueError(f"Access denied: {error_msg}")
        ...
        try:
            ...
            logger.error("Prompt get failed for '%s' on backend '%s': %s", 
                        actual_prompt_name, backend_name, e)
            raise
...
async def _setup_aggregated_logging(
    ...
    async def _set_logging_level(req: types.SetLevelRequest) -> types.ServerResult:
        ...
        try:
            ...
            results.append(f"proxy: success")
            logger.info("Proxy logging level set to %s", level_str.upper())
        ...
        for backend_name, session in backend_sessions.items():
            ...
            if backend_name in _backends_without_logging_support:
                logger.debug("Skipping backend '%s' - known to not support setLevel", backend_name)
            ...
            try:
                ...
                if ("Missing handler for request type: logging/setLevel" in error_str or
                    ...
                    logger.warning("Backend '%s' claims logging support but doesn't implement setLevel handler - adding to exclusion list", backend_name)
                    ...
                    if was_empty:
                        logger.info("Started dynamic exclusion list for backends without actual logging support")
                    results.append(f"{backend_name}: no setLevel handler (excluded)")
                elif "not active" in error_str.lower() or "session" in error_str.lower():
                    logger.warning("Backend '%s' session issue during logging request: %s", backend_name, error_str)
                    results.append(f"{backend_name}: session issue")
                else:
                    logger.error("Failed to set logging level on backend '%s': %s", backend_name, e)
                    results.append(f"{backend_name}: failed - {e}")
        
        logger.info("Set logging level to %s on %d targets: %s", 
                   req.params.level, len(results), ", ".join(results))
        
        # Log exclusion list status if it has entries
        if _backends_without_logging_support:
            logger.info("Backends excluded from logging requests: %s", list(_backends_without_logging_support))
        
        return types.ServerResult(types.EmptyResult())
    
    async def _set_logging_level_wrapper(req: types.SetLevelRequest) -> types.ServerResult:
        ...
        try:
            ...
            if ("Missing handler for request type: logging/setLevel" in error_str or
                ("HTTP 500" in error_str and "logging/setLevel" in error_str) or
                ("Failed to handle request" in error_str and "logging/setLevel" in error_str)):
                logger.warning("Backend reported logging capability but doesn't implement setLevel handler - this should be investigated")
                logger.info("This backend will be automatically excluded from future logging requests")
            ...
            logger.warning("Returning success despite backend errors to prevent VS Code connection issues")
            return types.ServerResult(types.EmptyResult())
