...
class StartupMitigation:
    ...
    async def apply_startup_mitigations(
        ...
        """
        logger.info("Applying startup mitigations for %d backends...", len(backend_params))
        ...
        if mcp_remote_backends:
            logger.info("Found %d mcp-remote backends: %s", len(mcp_remote_backends), ", ".join(mcp_remote_backends))
            mitigation_results["mcp_remote_prep"] = await StartupMitigation.prepare_mcp_remote_environment()
        
        if java_backends:
            logger.info("Found %d Java backends: %s", len(java_backends), ", ".join(java_backends))
            mitigation_results["java_verify"] = await StartupMitigation.verify_java_environment(jar_paths)
        
        # Log mitigation summary
        successful_mitigations = sum(1 for result in mitigation_results.values() if result)
        total_mitigations = len(mitigation_results)
        
        logger.info("Startup mitigations completed: %d/%d successful", successful_mitigations, total_mitigations)
