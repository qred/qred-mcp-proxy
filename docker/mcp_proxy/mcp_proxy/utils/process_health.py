    ...
    async def check_http_health(
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                response = await client.get(params.url, headers={"Accept": "application/json"})
                if response.status_code in [200, 404, 405]:  # 404/405 are acceptable for MCP endpoints
                    return True, None
                else:
    ...
    async def check_stdio_health(
        ...
        try:
            ...
            if params.command == "npx" and params.args and "mcp-remote" in params.args:
                return await ProcessHealthChecker._check_mcp_remote_health(params, timeout)
            
            # For other commands, do a basic process start check
            return await ProcessHealthChecker._check_basic_process_health(params, timeout)
    ...
    async def _check_mcp_remote_health(
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                params.command, *params.args, "--help",
...
async def validate_backend_processes(
    backend_params: Dict[str, ServerParameters],
    ...
    async def check_single_backend(name: str, params: ServerParameters):
        async with semaphore:
            ...
            if is_healthy:
                ...
                logger.warning("Backend '%s' health check failed (%.2fs): %s", name, duration, error)
            
            return name, (is_healthy, error, duration)
    
    # Run all health checks concurrently
    tasks = [
        check_single_backend(name, params)
