...
class StartupMonitor:
    ...
    def end_phase(self, phase_name: str) -> None:
        ...
        start_time = self.phase_times.get(f"{phase_name}_start", self.startup_start_time)
    ...
    def record_backend_health_check(
        ...
        if passed:
            logger.debug("✓ Backend '%s' health check passed (%.2fs)", backend_name, duration)
        else:
            logger.warning("✗ Backend '%s' health check failed (%.2fs): %s", backend_name, duration, error)
    ...
    def record_session_result(
        ...
        if backend_name in self.backend_statuses:
            ...
            if success:
                logger.info("✓ Backend '%s' session created successfully (%.2fs, %d recoveries)", 
                           backend_name, duration or 0, recovery_attempts)
            else:
                logger.error("✗ Backend '%s' session creation failed (%.2fs, %d recoveries): %s", 
                            backend_name, duration or 0, recovery_attempts, error)
    ...
    def generate_startup_report(self) -> str:
        ...
        # Count statuses
        healthy_count = sum(1 for s in self.backend_statuses.values() if s.final_status == "healthy")
        failed_count = sum(1 for s in self.backend_statuses.values() if s.final_status == "failed")
        skipped_count = sum(1 for s in self.backend_statuses.values() if s.final_status == "skipped")
        ...
        if self.backend_statuses:
            ...
            for backend_name, status in self.backend_statuses.items():
                icon = {"healthy": "✓", "failed": "✗", "skipped": "⏭", "unknown": "?"}[status.final_status]
                ...
                if status.health_check_duration > 0:
                    ...
                    report_lines.append(f"    Health Check: {hc_status} ({status.health_check_duration:.2f}s)")
                    if status.health_check_error:
                        report_lines.append(f"    Health Error: {status.health_check_error}")
                ...
                if status.session_duration is not None:
                    ...
                    report_lines.append(f"    Session: {session_status} ({status.session_duration:.2f}s)")
                    if status.recovery_attempts > 0:
                        report_lines.append(f"    Recovery Attempts: {status.recovery_attempts}")
                    if status.session_error:
                        report_lines.append(f"    Session Error: {status.session_error}")
        ...
        if failed_count > 0:
            report_lines.append("  ⚠️  Some backends failed - check logs above for details")
        
        if skipped_count > 0:
            report_lines.append("  ℹ️  Some backends were skipped due to health check failures")
    ...
    def should_continue_startup(self) -> Tuple[bool, str]:
        ...
        failed_count = sum(1 for s in self.backend_statuses.values() if s.final_status == "failed")
        return True, f"{healthy_count}/{total_count} backends are healthy ({failed_count} failed)"
