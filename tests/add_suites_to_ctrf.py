...
def extract_suite_from_test_name(test_name: str) -> str:
    ...
    if match:
        ...
        suite_name = re.sub(r'([A-Z])([A-Z][a-z])', r'\1 \2', class_name)  # Handle sequences like "MCPProxy"
        suite_name = re.sub(r'([a-z])([A-Z])', r'\1 \2', suite_name)       # Handle normal CamelCase
        suite_name = suite_name.strip()
        
        # Clean up specific patterns after the split
        suite_name = suite_name.replace('O Auth', 'OAuth')     # Fix split OAuth back to OAuth
