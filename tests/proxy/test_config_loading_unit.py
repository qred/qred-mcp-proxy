...
class TestConfigLoadingUnit:
    ...
    def test_oauth_config_loading_logic_empty_env(self):
        ...
        if not oauth_json.strip():
            with pytest.raises(
                ValueError, match="GOOGLE_OAUTH environment variable is empty"
            ):
    ...
    def test_oauth_config_loading_logic_invalid_client_id(self):
        ...
        config = {
            "web": {"client_id": "invalid-client-id", "client_secret": "valid-secret"}
        }
        oauth_json = json.dumps(config)
    ...
    def test_oauth_config_loading_logic_short_client_secret(self):
        ...
        config = {
            "web": {"client_id": "test.googleusercontent.com", "client_secret": "short"}
        }
        oauth_json = json.dumps(config)
    ...
    def test_mcp_servers_config_loading_logic_no_groups(self):
        ...
        config = {
            "mcpServers": {
                "no-groups-server": {"command": "python", "args": ["-m", "test_server"]}
            }
        }
...
class TestEnvironmentVariables:
    ...
    def test_empty_refresh_intervals(self):
        ...
        with patch.dict(
            ...
            groups_interval = int(
                os.getenv("MCP_OAUTH_REFRESH_GROUPS_INTERVAL", "15") or "15"
            )
            users_interval = int(
                os.getenv("MCP_OAUTH_REFRESH_USERS_INTERVAL", "60") or "60"
            )
