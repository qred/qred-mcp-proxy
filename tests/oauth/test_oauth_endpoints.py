...
class TestOAuthEndpoints:
    ...
    @patch(
        "mcp_oauth.server.valid_mcp_callbacks",
        ["https://claude.ai/api/mcp/auth_callback", "http://127.0.0.1:33418"],
    )
    ...
    def test_dynamic_client_registration_missing_redirect_uris(self, client):
        ...
        registration_request = {"client_name": "Test Client"}
    ...
    def test_dynamic_client_registration_no_credentials(self, client):
        ...
        assert (
            "Dynamic Client Registration credentials not configured"
    ...
    def test_oauth_token_authorization_code_missing_credentials(self, client):
        ...
        )  # Server returns invalid_client for missing credentials
    ...
    def test_oauth_callback_error(self, client):
        ...
        response = client.get(
            "/oauth/auth_callback?error=access_denied&error_description=User+denied+access"
        )
        assert response.status_code == 400
    ...
    @patch(
        "mcp_oauth.server._callback_forwarding",
        {"test-session": ("http://localhost:8080/callback", 1234567890)},
    )
    def test_oauth_callback_missing_session(self, client):
        """Test OAuth callback with missing session correlation."""
        response = client.get(
            "/oauth/auth_callback?code=test-code&state=unknown-session"
        )
        assert response.status_code == 400
